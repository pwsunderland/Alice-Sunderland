...
import sunderland.leapmotion.SwipeCircleNavigationListener;
...
public class LeapMotionSwipeCircleNavigationBehavior extends InternalResponseBehavior {

	private RenderTarget renderTarget;
	
	private Vector3d speed = new Vector3d(0,0,0);
	private Vector3d turning = new Vector3d(0,0,0);
	...
	private SwipeCircleNavigationListener swipeCircleNavigationListener = new SwipeCircleNavigationListener(this, controller);
	
	protected void disable() {
		renderTarget.removeKeyListener(keyMapping);
	}
	
	protected void enable() {
		renderTarget.addKeyListener(keyMapping);
	}
	
	public void keyPressed(KeyEvent keyEvent) {
		keyMapping.keyPressed(keyEvent);
	}
	
	public void keyReleased(KeyEvent keyEvent) {
		keyMapping.keyReleased(keyEvent);
	}
	
	public void started(World world, double time) {
		controller.addListener(swipeCircleNavigationListener);
		...
		if (isEnabled.booleanValue()) {
			...
			keyMapping = (KeyMapping)keyMap.getElementValue();
			keyMapping.cleanState();
//			((KeyMapping)keyMap.get()).cleanState();
	...
	public void stopped(World world, double time) {
		controller.removeListener(swipeCircleNavigationListener);
